...
. "$PSScriptRoot/../support/BggTierHelpers.ps1"
...
function Get-ReorderedRowsFromPubMeeple {
    ...
    foreach ($aliasKey in $TitleAliases.Keys) {
        $targetKey = [string]$TitleAliases[$aliasKey]
        if (-not $rankingByKey.ContainsKey($aliasKey) -and $rankingByKey.ContainsKey($targetKey)) {
            $rankingByKey[$aliasKey] = $rankingByKey[$targetKey]
        }
    }
    ...
    foreach ($row in $importRows) {
        ...
        if (-not $rankingByKey.ContainsKey($key)) {
            # A previously exported edition can disappear from this tier after
            # equivalence grouping. Its old row is obsolete here; the grouped
            # primary will be ranked in its current tier.
            if ($TitleAliases.ContainsKey($key)) {
                continue
            }
        ...
        $matchedRow = $rankingByKey[$key]
        $canonicalKey = Get-NormalizedTitleKey -Value ([string]$matchedRow.$RankingTitleColumn)
...
foreach ($item in $membership) {
    $targetKey = Get-NormalizedTitleKey -Value ([string]$item.name)
    foreach ($linkedId in @($item.linked_bgg_ids)) {
        $linked = $canonicalById[[int]$linkedId]
        if ($null -eq $linked) { continue }
        $aliasKey = Get-NormalizedTitleKey -Value ([string]$linked.name)
        if ($aliasKey -ne '' -and $aliasKey -ne $targetKey) {
            $titleAliases[$aliasKey] = $targetKey
        }
    }
}
...
foreach ($group in ($membership | Where-Object { $_.tier -ne 'U' } | Group-Object ranking_group)) {
    ...
    $pubMeepleImport = Get-ReorderedRowsFromPubMeeple -RankingRows $rows -PubMeepleCsvPath $pubMeepleFilePath -TitleAliases $titleAliases
