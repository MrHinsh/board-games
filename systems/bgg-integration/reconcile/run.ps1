...
$repoCanonicalPath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../../data/working/canonical/games.json'))
$requestedCanonicalPath = [IO.Path]::GetFullPath($CanonicalPath)
if ($requestedCanonicalPath -eq $repoCanonicalPath) {
...
# Older canonical-only entries may not appear in the current BGG collection
# snapshot. Keep the canonical contract complete even when no fresh metadata is
# available for those entries.
foreach ($game in $canonical) {
    foreach ($field in @('reimplements', 'reimplemented_by')) {
        if (-not $game.PSObject.Properties[$field]) {
            Set-ObjectProperty -Target $game -Name $field -Value @()
        }
    }
}
...
$safeFields = @('num_plays','bgg_rating','num_ratings','complexity','players','categories','mechanics','reimplements','reimplemented_by','year_published','name','collection','previously_owned','want_to_play','want_to_buy','collection_to_exit','collection_status','bgg_comment')
...
foreach ($freshGame in $fresh) {
    ...
    if ($canonicalByBggId.ContainsKey($id)) {
        ...
        $newEntry = [pscustomobject]@{
            ...
            reimplements   = @($freshGame.reimplements)
            reimplemented_by = @($freshGame.reimplemented_by)
