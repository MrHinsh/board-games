...
- `reimplements`: integer array of earlier BGG games this game reimplements
- `reimplemented_by`: integer array of later BGG games that reimplement this game
...
- `reimplements` and `reimplemented_by` default to empty arrays. Direction is
  taken from BGG's `boardgameimplementation` link and its `inbound` attribute;
  related implementations are not automatically treated as equivalent editions.
...
- `data/publish/pubmeeple/in/tier-*-ranking.txt` — generated by cross-system Rebuild.
- `data/raw/pubmeeple/in/tier-*-ranking.txt` — legacy normalize default; existing raw files must be preserved.
- `data/raw/pubmeeple/out/tier-*-ranking.csv`

Rules:

- normalize exports ranked tier names as newline-delimited text for PubMeeple input. Rebuild explicitly supplies `data/publish/pubmeeple/in`; direct legacy normalize retains its old default and callers should supply the publish path to avoid overwriting raw files.
