...
$needRefresh = $RefreshDesignerIndex.IsPresent
if (-not (Test-Path $DesignerIndexPath)) {
    $needRefresh = $true
} elseif (((Get-Date) - (Get-Item $DesignerIndexPath).LastWriteTime).TotalDays -gt $DesignerIndexMaxAgeDays) {
    $needRefresh = $true
}
...
if (-not $needRefresh) {
    ...
    # a canonical row added since the cache was built still needs fetching
    $missing = @($train | Where-Object { -not $designerById.ContainsKey([int]$_.bgg_id) })
    if ($missing.Count -gt 20) { $needRefresh = $true }
}

if ($needRefresh) {
    Write-Host "Building designer index for $($train.Count) rated games (this hits BGG; cached for $DesignerIndexMaxAgeDays days)..."
    $ids = @($train | ForEach-Object { [int]$_.bgg_id } | Sort-Object -Unique)
    $designerById = @{}
    for ($i = 0; $i -lt $ids.Count; $i += 20) {
        $batch = $ids[$i..([Math]::Min($i + 19, $ids.Count - 1))]
        $details = @(Invoke-BggMcpTool -ToolName 'bgg-details' -Arguments @{ ids = $batch } -Endpoint $Endpoint)
        foreach ($d in @($details | Where-Object { $_ -and $_.PSObject.Properties.Name -contains 'id' })) {
            # the details payload returns designers as one comma-joined string
            $designerById[[int]$d.id] = @((([string]$d.designer) -split ',\s*') | Where-Object { $_ })
        }
        Start-Sleep -Seconds 2
    }
    $dir = Split-Path -Parent $DesignerIndexPath
    if ($dir -and -not (Test-Path $dir)) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }
    $out = [ordered]@{}
    foreach ($k in ($designerById.Keys | Sort-Object)) { $out["$k"] = $designerById[$k] }
    $out | ConvertTo-Json -Depth 4 | Set-Content -Path $DesignerIndexPath -Encoding UTF8
    Write-Host "Designer index cached -> $DesignerIndexPath"
}
